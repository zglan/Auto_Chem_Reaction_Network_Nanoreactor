...
class G16_Set(SharedSetting):
    ...
    def setGjf(self):
        ...
        if self.job_type == 'opt':
            ...
            addit = self.irc_config["g16_addit"]
    ...
    def extractAtomChrg(self):
        for index, line in enumerate(self.log):
            if 'Mulliken charges:' in line:
                ...
                atomchrg = [float(i.split()[-2]) 
                    for i in self.log[index + 2: index + 1 + self.atomnums]]
        return atomchrg
    
    def extractSpinDensity(self):
        for index, line in enumerate(self.log):
            if len(self.atomlist) == 0:
                break
            if 'Mulliken charges and spin densities:' in line:
                spins = [float(i.split()[-1]) 
                    for i in self.log[index + 2: index + 1 + self.atomnums]]
        return spins
...
class G16_Rts(G16_Read): 
    ...
    def setIrcJob(self):
        ...
        G16_Set(
            gjf='%s-irc.gjf' % (self.jobname.split('.')[0]),
            job_type='irc',
            coord=atom_coord2str(self.extractCoord()[-1]),
            chrg=self.chrg,
            spin=self.spin).setGjf()
    
    def setTsRefJob(self):
        from ._toolkit import atom_coord2str
        G16_Set(
            gjf='%s-ts-ref.gjf' % (self.jobname.split('.')[0]),
            job_type='ts',
            coord=atom_coord2str(self.extractCoord()[-1]),
            chrg=self.chrg,
            spin=self.spin).setGjf()
