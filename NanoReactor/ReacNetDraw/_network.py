...
class ReacEveNet:
    ...
    def drawNet(self):
        print('Drawing _Network_Image......\n')

        G = self.__buildDiNet()
        pos = self.__setPos(G)
        G = self.__setNodeEdge(G, pos)

        node_degrees = [(node, G.out_degree(node) + G.in_degree(node), 
            G.out_degree(node), G.in_degree(node),
            list(G.neighbors(node)), list(G.predecessors(node))) 
                for node in G.nodes()]
        sorted_nodes = sorted(node_degrees, key=lambda x: x[1], reverse=True)
        for (node, degree, out_degree, in_degree, out_neigh, in_neigh) in sorted_nodes:
            out_neigh_str = ', '.join(map(str, out_neigh)) if out_neigh else "None"
            in_neigh_str = ', '.join(map(str, in_neigh)) if in_neigh else "None"
            print(f"Node {node}: Degree-{degree} (Out-{out_degree}, In-{in_degree}), Out Neighbors: [{out_neigh_str}], In Neighbors: [{in_neigh_str}]")
